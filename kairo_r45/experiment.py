...
ROOT = Path(__file__).resolve().parents[1]; DATA = ROOT / "datasets" / "r45"; RESULTS = ROOT / "results" / "r45"
...
def main():
    ...
    for policy in protocol()["policies"]:
        subprocess.run([sys.executable, "-m", "kairo_r43.benchmark", "--policy", policy, "--output", str(RESULTS / policy), "--data-dir", "datasets/r45"], cwd=ROOT, check=True)
