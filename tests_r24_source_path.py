...
class SourcePathTests(unittest.TestCase):
    ...
    def test_capability_ignores_inline_comment_require_text(self):
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder)
            (root / "main.js").write_text(
                'const label = "ordinary"; // require("socket")\nexport function run(value) { return value; }\n',
                encoding="utf-8",
            )
            result = Questioner(root, {}).ask("does this program use the network?")
            self.assertEqual(result["status"], "answered")
    ...
    def test_dependencies_preserve_code_after_url_string(self):
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder)
            (root / "main.js").write_text(
                'const endpoint = "https://example.test"; const net = require("node:net");\n',
                encoding="utf-8",
            )
            result = Questioner(root, {}).ask("what are the dependencies?")
            self.assertEqual(result["dependencies"], ["node:net"])
