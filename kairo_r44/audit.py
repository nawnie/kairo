...
ROOT = Path(__file__).resolve().parents[1]; RESULTS = ROOT / "results" / "r44_fast"
...
def main():
    ...
    report = {"experiment": "R44", "tasks": rows, "task_count": len(rows), "artifacts": sum(row["artifact"] for row in rows), "exact": sum(row["exact"] for row in rows), "replays": sum(row["replay"] for row in rows), "goal_achieved": False}
    out = ROOT / "verification" / "R44_AUDIT.json"; out.write_text(json.dumps(report, indent=2) + "\n")
