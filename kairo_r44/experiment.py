...
ROOT = Path(__file__).resolve().parents[1]; DATA = ROOT / "datasets" / "r44"; RESULTS = ROOT / "results" / "r44_fast"
...
def main():
    ...
    for policy in protocol()["policies"]:
        subprocess.run([sys.executable, "-m", "kairo_r43.benchmark", "--policy", policy, "--output", str(RESULTS / policy), "--data-dir", "datasets/r44"], cwd=ROOT, check=True)
