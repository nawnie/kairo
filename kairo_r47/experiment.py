...
ROOT = Path(__file__).resolve().parents[1]; DATA = ROOT / "datasets" / "r47"; RESULTS = ROOT / "results" / "r47_retry"
...
def protocol():
    ...
    value["learning"]["middle_depth"] = 1; value["learning"]["max_middle_depth"] = 1
    value["policies"] = ["coverage_backprop", "frontier_challenges", "random_challenges"]
...
def prepare():
    ...
    return {"cases": len(cases()), "policies": protocol()["policies"], "middle_depth": 1}
...
def main():
    ...
    for policy in protocol()["policies"]:
        subprocess.run([sys.executable, "-m", "kairo_r43.benchmark", "--policy", policy, "--output", str(RESULTS / policy), "--data-dir", "datasets/r47"], cwd=ROOT, check=True)
