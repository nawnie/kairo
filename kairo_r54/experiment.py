...
DATA = ROOT / "datasets" / "r54"
RESULTS = ROOT / "results" / "r54_retry3"
...
def prepare():
    ...
    protocol["learning"]["max_middle_depth"] = 2
    protocol["policies"] = ["random_challenges", "backprop_challenges", "structured_backprop_challenges"]
    protocol["action_budget"] = 16; protocol["state_cap"] = 64
    ...
    return {"cases": len(source_cases()), "policies": protocol["policies"], "data": str(DATA)}
...
def main():
    ...
    if args.command == "prepare":
        print(json.dumps(info, sort_keys=True)); return
    for policy in info["policies"]:
        subprocess.run([sys.executable, "-m", "kairo_r43.benchmark", "--policy", policy, "--output", str(RESULTS / policy), "--data-dir", "datasets/r54"], cwd=ROOT, check=True)
