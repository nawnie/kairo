...
ROOT = Path(__file__).resolve().parents[1]; DATA = ROOT / "datasets" / "r46"; RESULTS = ROOT / "results" / "r46"
...
def protocol():
    ...
    value["policies"] = ["frontier_challenges", "frontier_backprop", "random_challenges"]
...
def main():
    ...
    for policy in protocol()["policies"]:
        subprocess.run([sys.executable, "-m", "kairo_r43.benchmark", "--policy", policy, "--output", str(RESULTS / policy), "--data-dir", "datasets/r46"], cwd=ROOT, check=True)
